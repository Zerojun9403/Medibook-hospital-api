...
const methodMap: Record<string, string> = {
    ...
    CASH: "?íµ ?ÑÍ∏àÍ≤∞Ï†ú",
    TRANSFER: "?è¶ Í≥ÑÏ¢å?¥Ï≤¥",
    KAKAO_PAY: "?ü° Ïπ¥Ïπ¥?§Ìéò??,
    NAVER_PAY: "?ü¢ ?§Ïù¥Î≤ÑÌéò??,
};

const paymentStatusMap: Record<string, { label: string; color: string; bg: string }> = {
    PENDING: { label: "?ÄÍ∏?, color: "#e8a838", bg: "rgba(232,168,56,0.08)" },
    COMPLETED: { label: "?ÑÎ£å", color: "#2d9f6f", bg: "rgba(45,159,111,0.08)" },
    CANCELLED: { label: "Ï∑®ÏÜå", color: "#ef4444", bg: "rgba(239,68,68,0.08)" },
    REFUNDED: { label: "?òÎ∂à", color: "#6b7280", bg: "rgba(107,114,128,0.08)" },
};
...
async function apiFetch(endpoint: string, options: RequestInit = {}) {
    ...
    if (!res.ok) throw new Error(text ? JSON.parse(text).message || "?îÏ≤≠ ?§Ìå®" : "?îÏ≤≠ ?§Ìå®");
...
export default function PaymentPage() {
    ...
    const confirmPayment = async (paymentKey: string, orderId: string, amount: string) => {
        try {
            ...
            setPayResultMsg("Í≤∞Ï†úÍ∞Ä ?±Í≥µ?ÅÏúºÎ°??ÑÎ£å?òÏóà?µÎãà??");
            loadData();
        } catch (e: any) {
            setPayResult("fail");
            setPayResultMsg(e.message || "Í≤∞Ï†ú ?πÏù∏???§Ìå®?àÏäµ?àÎã§.");
        }
    };
    ...
    const handleTossPay = async (res: Reservation) => {
        if (!tossClientKey || !window.TossPayments) {
            alert("Í≤∞Ï†ú Î™®Îìà??Î∂àÎü¨?§Îäî Ï§ëÏûÖ?àÎã§. ?†Ïãú ???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.");
            return;
        }
        ...
        try {
            ...
            await tossPayments.requestPayment("Ïπ¥Îìú", {
                amount,
                orderId,
                orderName: `${res.departmentName} ÏßÑÎ£åÎπ?- ${res.doctorName}`,
            ...
            if (e.code !== "USER_CANCEL") {
                alert(e.message || "Í≤∞Ï†ú ?îÏ≤≠???§Ìå®?àÏäµ?àÎã§.");
            }
        } finally {
    ...
    const handleCancelPayment = async (paymentId: number) => {
        if (!confirm("Í≤∞Ï†úÎ•?Ï∑®ÏÜå?òÏãúÍ≤†Ïäµ?àÍπå?")) return;
        try {
            await apiFetch(`/api/payments/${paymentId}/cancel`, { method: "PATCH" });
            loadData();
        } catch (e: any) {
            alert(e.message || "Ï∑®ÏÜå???§Ìå®?àÏäµ?àÎã§.");
        }
    };
        ...
        <div className="min-h-screen bg-[var(--bg)]">
            ...
            <section className="bg-gradient-to-br from-primary-dark to-primary py-12 relative overflow-hidden">
                ...
                <div className="max-w-[1200px] mx-auto px-6 relative z-10">
                    ...
                    <h1 className="font-serif text-[32px] font-bold text-white mb-2">ÏßÑÎ£åÎπ?Í≤∞Ï†ú</h1>
                    <p className="text-[14px] text-white/50">?†Ïä§?òÏù¥Î®ºÏ∏†Î•??µÌïú ?àÏ†Ñ??Í≤∞Ï†ú</p>
                </div>
            </section>
            ...
            <div className="max-w-[1200px] mx-auto px-6 py-8">
                ...
                {payResult === "success" && (
                    <div className="mb-6 p-5 rounded-2xl bg-[rgba(45,159,111,0.06)] border border-[#2d9f6f]/20 flex items-center gap-4">
                        ...
                        <div>
                            <div className="text-[15px] font-bold text-[#2d9f6f]">Í≤∞Ï†ú ?ÑÎ£å!</div>
                ...
                {payResult === "fail" && (
                    <div className="mb-6 p-5 rounded-2xl bg-[rgba(239,68,68,0.06)] border border-[#ef4444]/20 flex items-center gap-4">
                        <div className="text-3xl">??/div>
                        <div>
                            <div className="text-[15px] font-bold text-[#ef4444]">Í≤∞Ï†ú ?§Ìå®</div>
                            <div className="text-[13px] text-[var(--text-light)]">{payResultMsg}</div>
                        </div>
                        <button onClick={() => setPayResult(null)} className="ml-auto text-[var(--text-muted)] bg-transparent border-none cursor-pointer text-lg">??/button>
                    </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                    {[
                        { icon: "?í≥", label: "ÎØ∏Í≤∞??ÏßÑÎ£å", value: `${unpaidReservations.length}Í±?, color: "#e8a838" },
                        { icon: "??, label: "Í≤∞Ï†ú ?ÑÎ£å", value: `${payments.filter((p) => p.status === "COMPLETED").length}Í±?, color: "#2d9f6f" },
                        { icon: "?í∞", label: "Ï¥?Í≤∞Ï†ú??, value: `${totalPaid.toLocaleString()}??, color: "#1b4d6e" },
                    ].map((stat, i) => (
                ...
                {activeTab === "pay" && (
                    <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
                        <div className="p-6 pb-4">
                            <h2 className="text-[16px] font-bold text-primary-dark">ÎØ∏Í≤∞??ÏßÑÎ£å</h2>
                            <p className="text-[12px] text-[var(--text-muted)] mt-0.5">ÏßÑÎ£å ?ÑÎ£å ??Í≤∞Ï†úÍ∞Ä ?ÑÏöî???àÏïΩ ¬∑ ?†Ïä§?òÏù¥Î®ºÏ∏† Í≤∞Ï†ú</p>
                        </div>
                        <div className="px-6 pb-6">
                            {unpaidReservations.length > 0 ? (
                                        ...
                                        <div key={res.id} className="flex items-center gap-4 p-4 rounded-xl bg-[var(--bg)] border border-[var(--border)]">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="text-[14px] font-bold text-primary-dark">{res.doctorName} ?ÑÎ¨∏??/span>
                                                ...
                                                <div className="text-[13px] font-bold text-accent mt-0.5">{(res.fee || 30000).toLocaleString()}??/div>
                                            </div>
                                            ...
                                            >
                                                ...
                                                {isProcessing ? "Ï≤òÎ¶¨Ï§?.." : "?†Ïä§Î°?Í≤∞Ï†ú"}
                                            </button>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-[14px] font-semibold text-primary-dark mb-1">ÎØ∏Í≤∞??ÏßÑÎ£åÍ∞Ä ?ÜÏäµ?àÎã§</p>
                                    <p className="text-[12px] text-[var(--text-muted)]">Î™®Îì† ÏßÑÎ£åÎπÑÍ? Í≤∞Ï†ú ?ÑÎ£å?òÏóà?µÎãà??/p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="mx-6 mb-6 p-4 rounded-xl bg-[#3182F6]/[0.04] border border-[#3182F6]/10">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <div className="text-[12px] font-semibold text-primary-dark mb-1">?†Ïä§?òÏù¥Î®ºÏ∏† ?àÏ†ÑÍ≤∞Ï†ú</div>
                                    <p className="text-[11px] text-[var(--text-light)] leading-[1.6]">
                                        Ïπ¥Îìú, Í∞ÑÌé∏Í≤∞Ï†ú ???§Ïñë??Í≤∞Ï†ú ?òÎã®??ÏßÄ?êÌï©?àÎã§. Î™®Îì† Í≤∞Ï†ú ?ïÎ≥¥???†Ïä§?òÏù¥Î®ºÏ∏†Î•??µÌï¥ ?îÌò∏?îÎêò???àÏ†Ñ?òÍ≤å Ï≤òÎ¶¨?©Îãà??
                                        <span className="text-[#3182F6] font-semibold"> (?åÏä§??Î™®Îìú)</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                {activeTab === "history" && (
                    <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
                        <div className="p-6 pb-4">
                            <h2 className="text-[16px] font-bold text-primary-dark">Í≤∞Ï†ú ?¥Ïó≠</h2>
                            <p className="text-[12px] text-[var(--text-muted)] mt-0.5">Ï¥?{payments.length}Í±?/p>
                        </div>
                        <div className="px-6 pb-6">
                            {payments.length > 0 ? (
                                <div className="flex flex-col gap-3">
                                            ...
                                            <div key={p.id} className="p-4 rounded-xl bg-[var(--bg)] border border-[var(--border)]">
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-[14px] font-bold text-primary-dark">{p.doctorName} ?ÑÎ¨∏??/span>
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                                                    <div>
                                                        <div className="text-[10px] text-[var(--text-muted)] mb-0.5">ÏßÑÎ£åÍ≥?/div>
                                                        <div className="text-[12px] text-[var(--text)]">{p.departmentName}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-[var(--text-muted)] mb-0.5">ÏßÑÎ£å??/div>
                                                        <div className="text-[12px] text-[var(--text)]">{p.reservationDate}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-[var(--text-muted)] mb-0.5">Í≤∞Ï†ú ?òÎã®</div>
                                                        <div className="text-[12px] text-[var(--text)]">{methodMap[p.method] || p.method}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-[var(--text-muted)] mb-0.5">Í≤∞Ï†ú Í∏àÏï°</div>
                                                        <div className="text-[14px] font-bold text-accent">{p.amount.toLocaleString()}??/div>
                                                    </div>
                                                </div>
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-[14px] text-[var(--text-muted)]">Í≤∞Ï†ú ?¥Ïó≠???ÜÏäµ?àÎã§</p>
                                </div>
                            )}
                        </div>
                    </div>
