...
const API_BASE = "hhttps://medibook-backend-1-zgb5.onrender.com";
        ...
        <div className="min-h-screen bg-[var(--bg)]">
            ...
            <div className="max-w-[1200px] mx-auto px-6 py-8">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-[28px] font-bold text-primary-dark mb-1">관리자 대시보드 🛡️</h1>
                    <p className="text-[14px] text-[var(--text-light)]">전체 시스템 현황을 확인하고 관리하세요</p>
                </div>

                {/* Stats */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    {[
                        { icon: "👥", label: "전체 회원", value: `${stats?.totalUsers ?? 0}명`, color: "#1b4d6e", bg: "rgba(27,77,110,0.06)" },
                        { icon: "📅", label: "전체 예약", value: `${stats?.totalReservations ?? 0}건`, color: "#e8a838", bg: "rgba(232,168,56,0.06)" },
                        { icon: "👨‍⚕️", label: "의사", value: `${doctorCount}명`, color: "#2a6f97", bg: "rgba(42,111,151,0.06)" },
                        { icon: "🏥", label: "환자", value: `${patientCount}명`, color: "#2d9f6f", bg: "rgba(45,159,111,0.06)" },
                    ].map((stat, i) => (
                ...
                {activeTab === "overview" && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white rounded-2xl border border-[var(--border)] p-6">
                            <h3 className="text-[15px] font-bold text-primary-dark mb-5">예약 현황</h3>
                            <div className="flex flex-col gap-4">
                                {[
                                    { label: "확정", count: confirmedCount, color: "#2d9f6f", total: reservations.length },
                                    { label: "완료", count: completedCount, color: "#2a6f97", total: reservations.length },
                                    ...
                                    <div key={i}>
                                        <div className="flex justify-between items-center mb-1.5">
                                            ...
                                            <span className="text-[13px] font-bold" style={{ color: item.color }}>{item.count}건</span>
                                        </div>
                        ...
                        {/* Today's Schedule */}
                        <div className="bg-white rounded-2xl border border-[var(--border)] p-6">
                            <h3 className="text-[15px] font-bold text-primary-dark mb-5">오늘의 예약</h3>
                            {todayReservations.length > 0 ? (
                                ...
                                <p className="text-[13px] text-[var(--text-muted)] text-center py-6">오늘 예약이 없습니다</p>
                            )}
                        </div>

                        {/* Recent Users */}
                        <div className="bg-white rounded-2xl border border-[var(--border)] p-6">
                            <h3 className="text-[15px] font-bold text-primary-dark mb-5">최근 가입 회원</h3>
                            <div className="flex flex-col gap-3">
                        ...
                        {/* Recent Reservations */}
                        <div className="bg-white rounded-2xl border border-[var(--border)] p-6">
                            <h3 className="text-[15px] font-bold text-primary-dark mb-5">최근 예약</h3>
                            <div className="flex flex-col gap-3">
                ...
                {activeTab === "users" && (
                    <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
                        <div className="flex items-center justify-between p-6 pb-4">
                            <div>
                                <h2 className="text-[16px] font-bold text-primary-dark">회원 목록</h2>
                                <p className="text-[12px] text-[var(--text-muted)] mt-0.5">총 {users.length}명</p>
                            </div>
                            <input
                                type="text"
                        ...
                        <div className="px-6 pb-6">
                            ...
                            {filteredUsers.length === 0 && (
                                <div className="text-center py-10">
                                    <p className="text-[14px] text-[var(--text-muted)]">검색 결과가 없습니다</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
                        <div className="flex items-center justify-between p-6 pb-4">
                            <div>
                                ...
                                <p className="text-[12px] text-[var(--text-muted)] mt-0.5">총 {reservations.length}건</p>
                            </div>
