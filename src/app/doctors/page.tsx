...
export default function DoctorsPage() {
    ...
    const [selectedDept, setSelectedDept] = useState("?ÑÏ≤¥");
    ...
    useEffect(() => {
        fetch(`${API_BASE}/api/doctors`)
            ...
            .then((data) => {
                ...
                setDepartments(["?ÑÏ≤¥", ...depts]);
            })
            .catch(() => setDoctors([]));
    }, []);

    const filtered = doctors.filter((d) => {
        const matchDept = selectedDept === "?ÑÏ≤¥" || d.departmentName === selectedDept;
    ...
    const getEmoji = (name: string) => {
        const female = ["?¥ÏàòÎØ?, "ÏµúÏ?ÏßÄ", "?úÏÑú??, "Í∞ïÎ???];
        return female.includes(name) ? "?ë©?ç‚öïÔ∏? : "?ë®?ç‚öïÔ∏?;
    };
        ...
        <div className="min-h-screen bg-[var(--bg)]">
            ...
            <section className="bg-gradient-to-br from-primary-dark to-primary py-16 relative overflow-hidden">
                ...
                <div className="max-w-[1200px] mx-auto px-6 relative z-10">
                    ...
                    <h1 className="font-serif text-[36px] font-bold text-white mb-3">?òÎ£åÏß??åÍ∞ú</h1>
                    <p className="text-[15px] text-white/50 max-w-[480px]">
                        Í∞?Î∂ÑÏïº ÏµúÍ≥†???ÑÎ¨∏?òÍ? ?ïÌôï??ÏßÑÎã®Í≥?ÏπòÎ£åÎ•??úÍ≥µ?©Îãà??                    </p>
                </div>
            </section>
            ...
            <div className="max-w-[1200px] mx-auto px-6 py-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                    <div className="relative flex-1 max-w-[360px]">
                        ...
                        <input
                            type="text"
                            placeholder="?òÎ£åÏß?Í≤Ä??(?¥Î¶Ñ, ?ÑÎ¨∏Î∂ÑÏïº...)"
                            value={searchQuery}
                ...
                <div className="text-[13px] text-[var(--text-muted)] mb-6">
                    Ï¥?<span className="text-primary font-bold">{filtered.length}</span>Î™ÖÏùò ?òÎ£åÏß?                </div>
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {doctor.available ? (
                                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-[rgba(45,159,111,0.08)] text-[#2d9f6f] font-semibold">ÏßÑÎ£å Í∞Ä??/span>
                                                ) : (
                                                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-[rgba(107,114,128,0.08)] text-[#6b7280] font-semibold">ÏßÑÎ£å ÎßàÍ∞ê</span>
                                                )}
                                            </div>
                                ...
                                <div className="p-6 pt-4">
                                    <div className="flex items-center gap-4 mb-4">
                                        ...
                                        <span className="text-[11px] text-[var(--text-muted)]">Î¶¨Î∑∞ {doctor.reviewCount}Í∞?/span>
                                        <span className="text-[11px] text-[var(--text-muted)]">¬∑</span>
                                        <span className="text-[11px] text-[var(--text-muted)]">Í≤ΩÎ†• {doctor.experienceYears}??/span>
                                    </div>

                                    {doctor.bio && (
                                        <p className="text-[12px] text-[var(--text-light)] leading-[1.7] mb-4 line-clamp-2">{doctor.bio}</p>
                                    )}
                                    ...
                                    >
                                        {doctor.available ? "?àÏïΩ?òÍ∏∞" : "?àÏïΩ ÎßàÍ∞ê"} <ChevronRight size={14} />
                                    </Link>
                                </div>
                            </div>
                        );
                ...
                {filtered.length === 0 && (
                    <div className="text-center py-16">
                        ...
                        <h3 className="text-[16px] font-bold text-primary-dark mb-2">Í≤Ä??Í≤∞Í≥ºÍ∞Ä ?ÜÏäµ?àÎã§</h3>
                        <p className="text-[13px] text-[var(--text-muted)]">?§Î•∏ Í≤Ä?âÏñ¥??ÏßÑÎ£åÍ≥ºÎ? ?†ÌÉù?¥Î≥¥?∏Ïöî</p>
                    </div>
                )}
            </div>
                    ...
                    <div className="relative bg-white rounded-2xl max-w-[520px] w-full max-h-[80vh] overflow-y-auto animate-scaleIn" onClick={(e) => e.stopPropagation()}>
                        <div className="bg-gradient-to-r from-primary/[0.04] to-accent/[0.04] p-8 relative">
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    ...
                                    <div className="text-[13px] text-[var(--text-light)]">{selectedDoctor.departmentName} ¬∑ {selectedDoctor.specialty}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8">
                            <div className="grid grid-cols-3 gap-3 mb-6">
                                {[
                                    { label: "?âÏ†ê", value: selectedDoctor.rating.toFixed(1), sub: `${selectedDoctor.reviewCount}Í∞?Î¶¨Î∑∞` },
                                    { label: "Í≤ΩÎ†•", value: `${selectedDoctor.experienceYears}??, sub: "?ÑÏÉÅ Í≤ΩÌóò" },
                                    { label: "?ôÎ†•", value: "?òÌïôÎ∞ïÏÇ¨", sub: selectedDoctor.education || "-" },
                                ].map((s, i) => (
                            ...
                            {selectedDoctor.bio && (
                                <>
                                    <h3 className="text-[13px] font-bold text-primary-dark mb-2">?åÍ∞ú</h3>
                            ...
                            {selectedDoctor.tags && selectedDoctor.tags.length > 0 && (
                                <>
                                    <h3 className="text-[13px] font-bold text-primary-dark mb-2">?ÑÎ¨∏ Î∂ÑÏïº</h3>
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setSelectedDoctor(null)} className="btn-outline flex-1 !py-3 !text-[13px]">?´Í∏∞</button>
                                {selectedDoctor.available && (
                                    <Link
