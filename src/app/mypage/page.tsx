...
const methodMap: Record<string, string> = {
    CARD: "?í≥ Ïπ¥Îìú", CASH: "?íµ ?ÑÍ∏à", TRANSFER: "?è¶ ?¥Ï≤¥",
    KAKAO_PAY: "?ü° Ïπ¥Ïπ¥?§Ìéò??, NAVER_PAY: "?ü¢ ?§Ïù¥Î≤ÑÌéò??,
};
...
export default function MyPage() {
    ...
    const [filterStatus, setFilterStatus] = useState("?ÑÏ≤¥");
    ...
    const handleCancel = async (id: number) => {
        if (!confirm("?àÏïΩ??Ï∑®ÏÜå?òÏãúÍ≤†Ïäµ?àÍπå?")) return;
        try {
            await apiFetch(`/api/reservations/${id}/cancel`, { method: "PATCH" });
            loadData();
        } catch {
            alert("Ï∑®ÏÜå???§Ìå®?àÏäµ?àÎã§.");
        }
    };

    const filteredReservations = filterStatus === "?ÑÏ≤¥"
        ? reservations
    ...
    const tabs: { key: Tab; label: string; icon: string; count?: number }[] = [
        { key: "reservations", label: "?àÏïΩ ?¥Ïó≠", icon: "?ìÖ", count: upcomingCount },
        { key: "prescriptions", label: "Ï≤òÎ∞© Í∏∞Î°ù", icon: "?íä", count: prescriptions.length },
        { key: "payments", label: "Í≤∞Ï†ú ?¥Ïó≠", icon: "?í≥", count: payments.length },
        { key: "profile", label: "???ïÎ≥¥", icon: "?ë§" },
    ];
            ...
            <div className="max-w-[1200px] mx-auto px-6 py-8">
                ...
                <div className="bg-white rounded-2xl border border-[var(--border)] p-6 mb-6">
                    <div className="flex items-center gap-5">
                        ...
                        <div className="flex-1">
                            <h1 className="text-[22px] font-bold text-primary-dark">{user?.name || "?¨Ïö©??}??/h1>
                            <p className="text-[13px] text-[var(--text-light)]">{user?.email} ¬∑ {user?.role === "PATIENT" ? "?ºÎ∞ò ?òÏûê" : user?.role}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-[var(--border)]">
                        <div className="text-center">
                            ...
                            <div className="text-[11px] text-[var(--text-muted)]">?ÑÏ≤¥ ?àÏïΩ</div>
                        </div>
                        <div className="text-center">
                            <div className="text-[18px] font-bold text-[#2d9f6f]">{prescriptions.length}</div>
                            <div className="text-[11px] text-[var(--text-muted)]">Ï≤òÎ∞© Í∏∞Î°ù</div>
                        </div>
                        <div className="text-center">
                            <div className="text-[18px] font-bold text-accent">{totalPaid.toLocaleString()}??/div>
                            <div className="text-[11px] text-[var(--text-muted)]">Ï¥?Í≤∞Ï†ú??/div>
                        </div>
                    </div>
                </div>
                ...
                {activeTab === "reservations" && (
                    <div className="animate-fadeIn">
                        <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
                            {["?ÑÏ≤¥", "CONFIRMED", "WAITING", "COMPLETED", "CANCELLED"].map((status) => {
                                const label = status === "?ÑÏ≤¥" ? "?ÑÏ≤¥" : statusMap[status]?.label || status;
                                return (
                                    <button
                                        key={status}
                                    ...
                                    <div key={res.id} className={`bg-white rounded-2xl border border-[var(--border)] p-5 transition-all ${isPast ? "opacity-70" : "hover:shadow-md"}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {res.symptom && (
                                                    <div className="text-[11px] text-[var(--text-muted)] mt-0.5">Ï¶ùÏÉÅ: {res.symptom}</div>
                                                )}
                                            </div>
                        ...
                        {filteredReservations.length === 0 && (
                            <div className="text-center py-16 bg-white rounded-2xl border border-[var(--border)]">
                                <div className="text-5xl mb-4">?ìã</div>
                                <h3 className="text-[16px] font-bold text-primary-dark mb-2">?àÏïΩ ?¥Ïó≠???ÜÏäµ?àÎã§</h3>
                                <p className="text-[13px] text-[var(--text-muted)] mb-4">?àÎ°ú???àÏïΩ??ÎßåÎì§?¥Î≥¥?∏Ïöî</p>
                                <Link href="/booking" className="btn-primary !py-2.5 !px-6 !text-[13px] no-underline">?àÏïΩ?òÍ∏∞</Link>
                            </div>
                        )}
                    </div>
                )}
                ...
                {activeTab === "prescriptions" && (
                    <div className="animate-fadeIn">
                        {prescriptions.length > 0 ? (
                            ...
                            <div className="text-center py-16 bg-white rounded-2xl border border-[var(--border)]">
                                <div className="text-5xl mb-4">?íä</div>
                                <h3 className="text-[16px] font-bold text-primary-dark mb-2">Ï≤òÎ∞© Í∏∞Î°ù???ÜÏäµ?àÎã§</h3>
                                <p className="text-[13px] text-[var(--text-muted)]">ÏßÑÎ£å ???òÏÇ¨Í∞Ä Ï≤òÎ∞©?òÎ©¥ ?¨Í∏∞???úÏãú?©Îãà??/p>
                            </div>
                        )}
                    </div>
                )}
                ...
                {activeTab === "payments" && (
                    <div className="animate-fadeIn">
                        {payments.length > 0 ? (
                            ...
                            <div className="text-center py-16 bg-white rounded-2xl border border-[var(--border)]">
                                <div className="text-5xl mb-4">?í≥</div>
                                <h3 className="text-[16px] font-bold text-primary-dark mb-2">Í≤∞Ï†ú ?¥Ïó≠???ÜÏäµ?àÎã§</h3>
                                <p className="text-[13px] text-[var(--text-muted)]">ÏßÑÎ£å ?ÑÎ£å ??Í≤∞Ï†ú?òÎ©¥ ?¨Í∏∞???úÏãú?©Îãà??/p>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <div className="animate-fadeIn max-w-[600px]">
                        <div className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden">
                            <div className="p-6 border-b border-[var(--border)]">
                                <h3 className="text-[15px] font-bold text-primary-dark">Í∏∞Î≥∏ ?ïÎ≥¥</h3>
                            </div>
                            <div className="p-6">
                                <div className="flex flex-col gap-5">
                                    {[
                                        { label: "?¥Î¶Ñ", value: user?.name || "-" },
                                        { label: "?¥Î©î??, value: user?.email || "-" },
                                        { label: "?åÏõê ?†Ìòï", value: user?.role === "PATIENT" ? "?ºÎ∞ò ?òÏûê" : user?.role || "-" },
                                    ].map((item, i) => (
                        ...
                        <div className="mt-6 p-5 rounded-2xl bg-primary/[0.03] border border-primary/[0.08]">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <div className="text-[12px] font-semibold text-primary-dark mb-1">Í∞úÏù∏?ïÎ≥¥ Î≥¥Ìò∏</div>
                                    <p className="text-[11px] text-[var(--text-light)] leading-[1.6]">
                                        ?åÏõê?òÏùò Í∞úÏù∏?ïÎ≥¥??BCryptÎ°??îÌò∏?îÎêò???àÏ†Ñ?òÍ≤å Î≥¥Ìò∏?©Îãà??
                                        ÎπÑÎ?Î≤àÌò∏ Î≥ÄÍ≤ΩÏù¥??Í≥ÑÏ†ï Í¥Ä??Î¨∏Ïùò??Í≥†Í∞ù?ºÌÑ∞Î°??∞ÎùΩ?¥Ï£º?∏Ïöî.
                                    </p>
                                </div>
                            </div>
                        </div>
